...
#if QLZ_COMPRESSION_LEVEL == 1 && defined QLZ_PTR_64 && \
...
static __inline void memcpy_up(unsigned char* dst,
...
static __inline void update_hash(qlz_state_decompress* state,
...
#if QLZ_COMPRESSION_LEVEL <= 2
static void update_hash_upto(qlz_state_decompress* state,
                             unsigned char** lh,
...
static size_t qlz_decompress_core(const unsigned char* source,
                                  unsigned char* destination,
                                  size_t size,
    ...
    static const ui32 bitlut[16] = {4, 0, 1, 0, 2, 0, 1, 0,
                                    3, 0, 1, 0, 2, 0, 1, 0};
...
size_t qlz_compress(const void* source,
...
#endif
    {
        ...
        r = base + qlz_compress_core(src, (unsigned char*)destination + base,
                                     size, state);
...
size_t qlz_decompress(const char* source,
                      void* destination,
                      qlz_state_decompress* state) {
    size_t dsiz = qlz_size_decompressed(source);

#if QLZ_STREAMING_BUFFER > 0
    if (state->stream_counter + qlz_size_decompressed(source) - 1 >=
...
#endif
    {
        ...
        if ((*source & 1) == 1) {
            dsiz = qlz_decompress_core(
                (const unsigned char*)source, dst, dsiz, state,
