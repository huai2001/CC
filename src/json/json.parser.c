...
_CC_API_PRIVATE(tchar_t*) _json_parser_string(_cc_sbuf_tchar_t *const buffer) {
    ...
    while (*p && ((size_t)(p - buffer->content) < buffer->length) && (*p) != isquoted) {
        /* is escape sequence */
        if (*p == _T('\\')) {
    ...
    if (_cc_unlikely(((size_t)(p - buffer->content) >= buffer->length) || (*p) != isquoted)) {
        goto JSON_FAIL;
    }
...
_CC_API_PRIVATE(bool_t) _json_parser_number(_cc_sbuf_tchar_t *const buffer, _cc_json_t *const item) {
    ...
    if (num.vt == _CC_NUMBER_INT_) {
        ...
        item->object.uni_int = (int64_t)num.v.uni_int;
    } else {
        item->type = _CC_JSON_FLOAT_;
        item->object.uni_float = (float64_t)num.v.uni_float;
    }
...
_CC_API_PRIVATE(bool_t) _json_parser_key_and_value(_cc_sbuf_tchar_t *const buffer, _cc_json_t *item) {
    ...
    if (_cc_unlikely(!_cc_sbuf_access(buffer) || (*_cc_sbuf_offset(buffer) != _T(':')))) {
        _cc_free(item->name);
        return false;
    }
    /* skip : */
...
_CC_API_PRIVATE(bool_t) _json_parser_object(_cc_sbuf_tchar_t *const buffer, _cc_json_t *item) {
    ...
    if (_cc_unlikely(*_cc_sbuf_offset(buffer) == _JSON_OBJECT_END_)) {
        goto JSON_SUCCESS;
    }
    ...
    do {
        /*parse next value */
        buffer->offset++;
        if (_cc_buf_jump_comments(buffer) && (*_cc_sbuf_offset(buffer) == _JSON_OBJECT_END_))
            break;
        ...
        if (_cc_unlikely(!_cc_rbtree_push(&item->object.uni_object, &curr_item->node, _json_push_object))) {
            _cc_destroy_json(&curr_item);
            goto JSON_FAIL;
        }
    } while (_cc_sbuf_access(buffer) && (*_cc_sbuf_offset(buffer) == _JSON_NEXT_TOKEN_));

    /*expected end of object*/
    if (!_cc_sbuf_access(buffer) || (*_cc_sbuf_offset(buffer)) != _JSON_OBJECT_END_) {
        goto JSON_FAIL;
    }
...
_CC_API_PRIVATE(bool_t) _json_parser_array(_cc_sbuf_tchar_t *const buffer, _cc_json_t *item) {
    ...
    /* check if we skipped to the end of the buffer */
    if (_cc_unlikely(!_cc_buf_jump_comments(buffer))) {
        buffer->offset--;
        goto JSON_FAIL;
    }

    if (_cc_unlikely(*_cc_sbuf_offset(buffer) == _JSON_ARRAY_END_)) {
        goto JSON_SUCCESS;
    }
    ...
    do {
        /*parse next value */
        buffer->offset++;
        if (_cc_buf_jump_comments(buffer) && (*_cc_sbuf_offset(buffer) == _JSON_ARRAY_END_)) {
            break;
        }
        ...
        if (_cc_unlikely(_cc_array_push(&item->object.uni_array, curr_item) == -1)) {
            _cc_destroy_json(&curr_item);
            goto JSON_FAIL;
        }
    } while (_cc_sbuf_access(buffer) && (*_cc_sbuf_offset(buffer) == _JSON_NEXT_TOKEN_));
    ...
    /*expected end of object*/
    if (!_cc_sbuf_access(buffer) || (*_cc_sbuf_offset(buffer)) != _JSON_ARRAY_END_) {
        goto JSON_FAIL;
    }
...
_CC_API_PRIVATE(bool_t) _json_read(_cc_sbuf_tchar_t *const buffer, _cc_json_t *item) {
    ...
    if (_cc_sbuf_access(buffer)) {
        ...
        item->object.uni_string = _json_parser_string(buffer);
        if (item->object.uni_string) {
...
_CC_API_PUBLIC(_cc_json_t*) _cc_josn_parser(_cc_sbuf_tchar_t *const buffer) {
    ...
    item->type = _CC_JSON_NULL_;
    if (_cc_buf_jump_comments(buffer) && _json_read(buffer, item)) {
        return item;
    }
