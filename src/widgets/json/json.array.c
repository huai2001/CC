...
/**/
_CC_API_PUBLIC(_cc_json_t*) _cc_json_alloc_array(byte_t type, size_t size) {
    _cc_json_t *ctx = (_cc_json_t *)_cc_malloc(sizeof(_cc_json_t));
    bzero(ctx, sizeof(_cc_json_t));
    ctx->type = type;
