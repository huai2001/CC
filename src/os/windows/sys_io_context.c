...
void _io_context_free(_cc_async_event_priv_t *priv, _io_context_t *io_context) {
    ...
    if (priv->frees >= _CC_MAX_CHANGE_EVENTS_) {
        _cc_list_remove(&io_context->lnk);
        _cc_free(io_context);
        return;
    }

    _cc_list_swap(&priv->io_idle, &io_context->lnk);
    priv->frees++;
    return;
}
