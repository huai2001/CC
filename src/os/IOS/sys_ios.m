 ...
 * @note On iOS, this returns the overall system CPU usage, not just the current app's usage
 */
 ...
 * @note On iOS, this returns device-wide memory usage, not just the current app's memory usage
 */
_CC_API_PUBLIC(void) _cc_get_memory_usage(double* total, double* used) {
    ...
    if (used != NULL) {
        ...
        /* Use host_statistics64 for VM statistics on iOS */
