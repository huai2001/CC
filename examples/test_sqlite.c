...
const tchar_t *createTable1 = "create table if not exists FileList1 (" \
        "`check` VARCHAR(65) PRIMARY KEY NOT nullptr," \
...
const tchar_t *createTable3 = "create table if not exists FileList (" \
        ...
        "`path` TEXT NOT nullptr," \
        "`upload` INTEGER default(0)," \
        "`update_time` timestamp NOT nullptr default(datetime('now','localtime'))" \
    ")";
bool_t scanFile(const tchar_t *directory, _cc_sql_result_t *result) {
    ...
    while ((d = readdir(dpath)) != nullptr) {
        ...
        if (S_ISDIR(stat_buf.st_mode) == 0) {
            ...
            sql_delegate.bind(result, 1, &stat_buf.st_size, sizeof(int64_t), _CC_SQL_TYPE_INT64_);
...
int main(int argc, char *const arvg[]) {
    ...
    sql_delegate.execute(sql, _T("delete from `FileList1`;"), nullptr);
    sql_delegate.execute(sql, _T("delete from `FileList2`;"), nullptr);

    if (sql_delegate.prepare(sql, _T("INSERT INTO `FileList1` (`check`, `size`, `path`) VALUES (?,?,?);"), &sql_result)) {
        scanFile("./", sql_result);
        sql_delegate.free_result(sql, sql_result);
    }
    
    sql_delegate.execute(sql, "REPLACE INTO `FileList2` (`check`, `size`, `path`,`upload`) SELECT f1.`check`, f1.`size`, f1.`path`,f2.`upload` FROM FileList1 as f1 left join FileList as f2 on f1.`check`=f2.`check`;", nullptr);
    sql_delegate.execute(sql, "REPLACE INTO `FileList`(`check`, `size`, `path`,`upload`) SELECT `check`, `size`, `path`, `upload` FROM FileList2;", nullptr);
    sql_delegate.execute(sql, _T("UPDATE `FileList` SET `upload`=0 WHERE `upload` is nullptr;"), nullptr);
    sql_delegate.execute(sql, _T("delete from `FileList1`;"), nullptr);
    sql_delegate.execute(sql, _T("delete from `FileList2`;"), nullptr);
    ...
    sql_delegate.begin_transaction(sql);
    if (sql_delegate.prepare(sql, _T("UPDATE `FileList` SET `upload`=? WHERE `check`=?;"), &sql_result)) {
    ...
    if (sql_delegate.execute(sql, "SELECT * FROM FileList;", &sql_result)) {
