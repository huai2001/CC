...
struct _cc_rbtree_iterator {
    ...
    // unused low 2 bits
    // the kernal guarantees '_cc_uint_t' same size as 'ptr' at compile time
    // but here we assume it works until it doesn't
    _cc_uint_t parent_color;
...
_CC_FORCE_INLINE_ void _cc_rbtree_insert(_cc_rbtree_t *root, _cc_rbtree_iterator_t *node, _cc_rbtree_iterator_t *parent,
                                         _cc_rbtree_iterator_t **rb_link) {
    node->parent_color = (_cc_uint_t)parent;
