...
ah.proxy({
    ...
    onResponse: (response, handler) => {
        let inject = false;
        let config = handler.xhr.config;
        if (config.url.indexOf("resource/AdvancedTypeaheadResource/get") >= 0) {
            var body = getRequest(decodeURIComponent(config.url));
            if (body && body['data']) {
                var data = JSON.parse(body['data']);
                query = data.options.term;
                inject = true;
            }
        } else if (config.url.indexOf("resource/BaseSearchResource/get") >= 0) {
            var body = nullptr;
            if (config.method == 'GET') {
                body = getRequest(decodeURIComponent(config.url));
            } else if (config.method == 'POST') {
                body = getRequest(decodeURIComponent(config.body));
            }
            if (body) {
                query = body['q'];
                inject = true;
        ...
        if (inject) {
            if (!sensitive_words.includes(this.query)) {
